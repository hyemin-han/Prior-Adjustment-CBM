...
bayes_adjusted<-img_data(readNIfTI('./HCP/transformed_BFs_80.nii'))>=3
result_bayes_meta<-as.matrix(c(
  calc_overlap(bayes_meta,bayes_adjusted)[1]
))
# neurosynth
...
bayes_adjusted<-img_data(readNIfTI('./HCP/transformed_BFs_85.nii'))>=3
result_bayes_meta<-as.matrix(c(
  calc_overlap(bayes_meta,bayes_adjusted)[1]
))
# neurosynth
...
bayes_adjusted<-img_data(readNIfTI('./HCP/transformed_BFs_95.nii'))>=3
result_bayes_meta<-as.matrix(c(
  calc_overlap(bayes_meta,bayes_adjusted)[1]
))
# neurosynth
...
write.csv(results,'wm_NeuroQuery_HCP.csv')
...
long_results$data<-'wm_HCP'
long_results$origin<-'NeuroQuery'
write.csv(long_results,'long_wm_NeuroQuery_HCP.csv')