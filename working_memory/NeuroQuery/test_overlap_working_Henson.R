...
fwe<-img_data(readNIfTI('./Henson/transformed_fwe.nii'))
bayes_meta<-img_data(readNIfTI('./transformed_Bayes_meta.hdr'))
bayes_meta[abs(bayes_meta)<1e-4]<-0
...
bayes_adjusted<-img_data(readNIfTI('./Henson/transformed_BFs_80.nii'))>=3
result_bayes_meta<-as.matrix(c(
  calc_overlap(bayes_meta,bayes_adjusted)[1]
))
# neurosynth
...
bayes_adjusted<-img_data(readNIfTI('./Henson/transformed_BFs_85.nii'))>=3
result_bayes_meta<-as.matrix(c(
  calc_overlap(bayes_meta,bayes_adjusted)[1]
))
# neurosynth
...
bayes_adjusted<-img_data(readNIfTI('./Henson/transformed_BFs_95.nii'))>=3
result_bayes_meta<-as.matrix(c(
  calc_overlap(bayes_meta,bayes_adjusted)[1]
))
# neurosynth
...
write.csv(results,'wm_NeuroQuery_Henson.csv')
...
long_results$data<-'wm_Henson'
long_results$origin<-'NeuroQuery'
write.csv(long_results,'long_wm_NeuroQuery_Henson.csv')