...
# automatic thresholding and false alarm rate (bf: default = 3)
calc_false_alarm3<-function(original, target, bf=3){
  # calculate the number of true actives and false actives in target
  true_act <- sum((original>0 & target >=bf))
  false_act <- sum((original==0 & target >= bf))
  
  false_alarm<-false_act/(true_act+false_act)
  return(false_alarm)
}
# with filename
calc_false_alarm_file<-function(original, target){
  # load files
...
# automatic thresholding and hit rate (bf: default = 3)
calc_hit_rate_3<-function(original,target, bf = 3){
  # both should be active
  true_act <- sum((original>0 & target >=bf))
  # also, all active voxels in the original image should be counted
  original_act <- sum(original >0)
...
# with filename
calc_hit_rate_file<-function(original,target){
  # load files
  original<-get_image(original)
  target<-get_image(target)
  # both should be active
